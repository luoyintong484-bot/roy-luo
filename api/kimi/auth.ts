...
function getJwks() {
  if (!_jwks) {
    const jwksUrl = `${env.kimiAuthUrl}/api/.well-known/jwks.json`;
    if (!/^https?:\/\//i.test(jwksUrl)) {
      throw new Error(
        `Invalid KIMI_AUTH_URL: "${env.kimiAuthUrl}". Must be an absolute URL with http/https prefix, e.g. https://open.moonshot.cn`
      );
    }
