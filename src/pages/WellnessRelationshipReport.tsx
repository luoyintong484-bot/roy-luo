...
export default function WellnessRelationshipReport() {
  ...
  const name1 = searchParams.get("name1") || (isZh ? "你" : "You");
  const name2 = searchParams.get("name2") || (isZh ? "對方" : "Partner");
  const sections = getRelationshipDynamicsSections(isZh);
  const reportKey = "wellness_relationship_dynamics";
    ...
    <div className="min-h-screen bg-[#0a0e12]">
      ...
      <main className="pt-20 pb-12 px-4 sm:px-6">
        <div className="max-w-2xl mx-auto">
          {/* Back */}
          <button
            onClick={() => navigate("/wellness/relationship")}
          ...
          <div className="text-center mb-8">
            <h1 className="font-display text-2xl font-bold text-[#f0e6d3] mb-2">
              {isZh ? `${name1} 與 ${name2} 的關係動力分析` : `Relationship Dynamics: ${name1} & ${name2}`}
            </h1>
            <p className="text-xs text-[#8a8aad]">
              {isZh ? "互動模式 · 溝通風格 · 成長空間" : "Interaction Patterns · Communication Styles · Growth Edges"}
            </p>
          </div>
